...
Don't ask Claude to "fix the whole project." Ask it to fix a single function with the minimum necessary context. `token-tengu` provides the tooling to make this surgical approach easier than the lazy one.
...
You should always see what is being sent before it goes. `token-tengu` is designed to be simple on the surface for "prompt demons" but fully inspectable underneath for "CLI discipline" enthusiasts.
